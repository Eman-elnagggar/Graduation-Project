...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  if (urlPatient) {
    const target = state.conversations.find(c => String(c.patientId) === String(urlPatient));
    if (target) {
...
function initializeState() {
  ...
  state.conversations = initial.map((c, idx) => ({
    id: c.id ?? idx + 1,
    patientId: String(c.patientId ?? c.id ?? ""),
    receiverUserId: String(c.receiverUserId ?? c.patientId ?? c.id ?? ""),
    name: c.name || "Patient",
    avatar: c.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(c.name || "P")}&background=14967f&color=fff&size=80`,
...
function setupEventListeners() {
  ...
  document.getElementById("viewProfileBtn")?.addEventListener("click", () => {
    ...
    window.location.href = `/Doctor/PatientDetails/${DOCTOR_ID}/${state.currentConversation.patientId}`;
  });
...
async function loadConversationMessages(conversation) {
  ...
  const endpoint = CONVERSATION_ENDPOINT_TEMPLATE
    ...
    .replace("__PATIENT_ID__", encodeURIComponent(conversation.patientId));
...
function handleIncomingMessage(senderId, message, sentAtUtc) {
  ...
  if (isSentByCurrentUser) {
    ...
    conversation = state.conversations.find(c => String(c.receiverUserId) === senderId || String(c.patientId) === senderId || String(c.id) === senderId);
  }
