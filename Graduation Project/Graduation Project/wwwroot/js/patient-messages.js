...
function renderMessage(msg) {
  ...
  if (msg.attachmentUrl) {
    if (msg.attachmentType === "image") {
      ...
      const icon = "📄";
      attachmentHtml = `<div class="msg-attachment msg-attachment-file"><a href="${escapeHtml(msg.attachmentUrl)}" target="_blank" rel="noopener" download="${escapeHtml(msg.attachmentName || "file")}">${icon} ${escapeHtml(msg.attachmentName || "Download file")}</a></div>`;
    }
  }
...
async function sendFileMessage(caption) {
  const uploadEndpoint = UPLOAD_ENDPOINT_TEMPLATE.replace("__PATIENT_ID__", encodeURIComponent(PATIENT_ID));
