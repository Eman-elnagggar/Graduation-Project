...
(function () {
  const oldKeys = ["assistantSidebarCollapsed", "doctorSidebarCollapsed"];
  oldKeys.forEach((key) => {
    if (localStorage.getItem(key) !== null) {
      // Migrate to new key if not already set
      if (localStorage.getItem("sidebarCollapsed") === null) {
        localStorage.setItem("sidebarCollapsed", localStorage.getItem(key));
      }
...
// ================================
function getStorageData(key, prefix = "") {
  const fullKey = prefix ? `${prefix}_${key}` : key;
  const data = localStorage.getItem(fullKey);
  return data ? JSON.parse(data) : null;
}

function setStorageData(key, data, prefix = "") {
  const fullKey = prefix ? `${prefix}_${key}` : key;
  localStorage.setItem(fullKey, JSON.stringify(data));
}

function removeStorageData(key, prefix = "") {
  const fullKey = prefix ? `${prefix}_${key}` : key;
